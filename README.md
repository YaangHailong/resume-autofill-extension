...
- 在扩展内维护一份结构化简历模板。
- 模板按新东方招聘表单结构设计，支持个人信息、求职意向、教育经历、工作经历、语言能力。
- 扫描网页中的 `input`、`textarea`、`select`、单选框、复选框、可编辑文本区域。
- 根据字段标签、占位提示、`name`、`id`、`aria-label`、邻近文本进行匹配。
- 内置字段语义匹配，能处理 `学历` / `学位` 这类页面命名不完全一致的字段。
- 在网页右上角显示填写预览，用户可以检查或调整字段映射。
- 确认后自动填写，不自动提交。
- 识别“添加教育经历”“添加工作经历”“添加语言能力”等动态添加按钮。
- 根据模板中多条教育/工作/语言记录，确认后自动点击添加按钮，再重新扫描并填写新增字段。
...
3. 在模板页面填写简历信息。
4. 至少填写姓名、电话、邮箱等基础字段。
5. 如果有多段教育经历、工作经历或语言能力，点击对应模块里的“添加”。
6. 点击右上角“保存”。
...
- 可参考 [docs/XDF_RESUME_PROFILE_TEMPLATE.json](docs/XDF_RESUME_PROFILE_TEMPLATE.json) 的模板结构。
...
- 需确认：有一定相似度，但建议人工检查。
- 未匹配：没有找到合适的网页字段。
- 动态添加：需要点击“添加教育经历/工作经历/语言能力”的次数。
...
4. 在扩展里保存一份包含教育、工作或语言能力的简历模板。
5. 点击“扫描当前页”。
6. 检查预览面板。
7. 点击“确认并填写”。
8. 确认姓名、手机号、邮箱、教育经历、工作经历等字段被写入。
...
- 扩展没有刷新到最新构建版本。
- 目标网页是在扩展刷新前打开的旧页面，里面还没有新版 content script。
- 如果打开的是本地 `file://` 文件，扩展可能没有开启“允许访问文件网址”。
...
4. 刷新目标招聘网页。
5. 如果是本地文件，在扩展详情页打开“允许访问文件网址”。
6. 再点击“扫描当前页”。

如果看到 `Could not establish connection. Receiving end does not exist.`，说明 popup 没找到当前页面里的 content script。新版会自动尝试注入并重试；如果仍失败，通常是当前页面不允许注入，或者本地文件访问权限没开。
...
不要把火山方舟 API key 写进 Chrome 扩展，扩展打包后的代码可以被查看，key 会泄露。接 AI 时需要准备一个后端服务，由扩展请求你的后端，后端再调用火山方舟 API。

需要准备：

- 后端接口地址。本地默认是 `http://127.0.0.1:8787/api/resume-field-match`。
- 后端环境变量，例如 `ARK_API_KEY`、`AI_MATCHING_MODEL`。
- `public/manifest.json` 中允许访问后端域名的 `host_permissions`。
- “启用 AI 增强匹配”的开关。
- 脱敏策略：只把字段标签、类型、下拉选项、区块信息发给 AI，尽量不发送手机号、邮箱等真实简历值。
...
启动后到扩展“简历模板”页面打开 `AI 增强匹配`，保存，再扫描网页。

更详细的接入说明见 [docs/AI_MATCHING_REQUIREMENTS.md](docs/AI_MATCHING_REQUIREMENTS.md)。

### 多条教育、工作或语言能力没有全部填上

确认目标网页是否有“添加教育经历”“添加工作经历”“添加语言能力”之类按钮。第一版会识别新东方页面这类自定义添加按钮，但对完全不同的按钮结构可能需要后续增加规则。
